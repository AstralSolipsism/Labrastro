...
当前 Taskflow 已有服务端状态模型、编译流程、派发端口、Postgres-backed snapshot 恢复和 HTTP 路由骨架，`dogcode` 侧已有 Taskflow chat mode 基础入口。完整产品化体验仍在建设中，包括 Review Cards 交互、自然澄清流程、Taskflow 详情页、项目级长期记忆可视化，以及工程产物自动生成/回写。
...
| 降级 | 无 `LABRASTRO_DATABASE_URL` 时，Taskflow / ProjectState 使用进程内状态或能力降级，重启后不可恢复。 |
