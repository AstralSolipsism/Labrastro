...
- **Postgres 控制面基础**：包含手写 Alembic 迁移、runtime/session/auth/taskflow/collaboration/GitHub PR 生命周期相关表结构。
...
当前 Taskflow 已有服务端状态模型、编译流程、派发端口和 HTTP 路由骨架。完整产品化体验仍在建设中，包括 Review Cards 前端交互、项目级长期记忆可视化、Postgres-backed Taskflow store wiring、工程产物自动生成/回写，以及从普通聊天自然进入 Taskflow 的用户旅程。
