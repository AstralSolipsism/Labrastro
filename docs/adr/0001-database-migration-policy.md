...
Labrastro uses optional Postgres persistence for runtime, auth, session, taskflow, collaboration, GitHub PR lifecycle, and related control-plane state. The current persistence layer is written around explicit SQL statements rather than SQLAlchemy ORM models.
