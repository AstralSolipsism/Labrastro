"""Pure-Python directory listing tool: read-only, no shell dependency."""
...
def _format_mode(mode: int) -> str:
    """Convert a stat mode to an ls-style permission string."""
...
class ListFileTool(Tool):
    name = "list_file"
    description = (
        "List files and directories. Pure read-only; no shell involved. "
        "Use this first for exploring project structure, checking what files exist, "
        "or verifying paths. Prefer this over `shell ls`; it is safer and returns "
        "stable output for the model."
    )
    parameters = {
        "type": "object",
        "properties": {
            ...
            "recursive": {
                ...
                "description": (
                    ...
                    "Entries are printed under each directory section."
                ),
            },
            "pattern": {
                "type": "string",
                "description": (
                    "Shell-style glob pattern to filter entries, e.g. '*.py'. "
                    "The filename is matched within each listed directory."
                ),
            },
        },
    }
    ...
    def _execute_remote(
        ...
        if not isinstance(all, bool):
            return "Error: all must be a boolean"
        if not isinstance(long, bool):
            return "Error: long must be a boolean"
    ...
    def _execute_local(
        ...
        if recursive:
            for subdir in sorted(self._collect_subdirs(resolved, all=all)):
    ...
    def _list_dir(
        ...
        *,
        ...
        if long:
            return [
                f"{_format_mode(mode)}  {size:>8}  {_format_mtime(mtime)}  "
